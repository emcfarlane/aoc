...
fn run(src: []const u8, allocator: std.mem.Allocator) !Result {
    ...
    while (lines.next()) |line| {
        ...
        for (line, 0..) |c, x| {
            ...
            try points.append(Point{ .x = @intCast(x), .y = y });
        }
        y += 1;
    }
    ...
    for (points.items) |point| {
        ...
        const isStar = isGear(grid.items, point);
        for (others) |other| {
            const x = findNumber(grid.items, other, &seen);
            if (x) |val| {
                result.total += val;
            }
        }
        if (isGear(grid.items, point)) {
            var seenGear = std.AutoHashMap(Point, void).init(allocator);
            defer seenGear.deinit();
            var gearRatio: u32 = 1;
            var valueCount: u32 = 0;
            for (others) |other| {
                const x = findNumber(grid.items, other, &seenGear);
                if (x) |val| {
                    gearRatio *= val;
                    valueCount += 1;
                }
            ...
            if (valueCount == 2) {
                std.debug.print("GOT GEAR RATIO: {any}\n", .{gearRatio});
                result.gearRatio += gearRatio;
            }
            std.debug.print("Point: {any} isStar: {any} valueCount: {any} gearRatio: {any}\n", .{ point, isStar, valueCount, gearRatio });
        }
    }
    //var keyIter = seen.keyIterator();
...
fn findNumber(grid: [][]const u8, point: Point, seen: *std.AutoHashMap(Point, void)) ?u32 {
    if (seen.contains(point)) {
        return null;
    }
    if (!isDigit(grid, point)) {
        return null;
    }
    seen.put(point, {}) catch unreachable;
    var left = point;
    left.x -= 1;
    while (isDigit(grid, left)) {
        seen.put(left, {}) catch unreachable;
        left.x -= 1;
    }
    left.x += 1;
    var right = point;
    right.x += 1;
    while (isDigit(grid, right)) {
        seen.put(right, {}) catch unreachable;
        right.x += 1;
    }
    right.x -= 1;
    if (left.x <= right.x) {
        const str = grid[@intCast(point.y)][@intCast(left.x)..@intCast(right.x + 1)];
        std.debug.print("Found number: {s} from {any}\n", .{ str, point });
        return std.fmt.parseInt(u32, str, 10) catch null;
    }
    return null;
}

fn getChar(grid: [][]const u8, point: Point) u8 {
    if (point.x < 0 or point.x >= grid[0].len) {
        return 0;
    }
    if (point.y < 0 or point.y >= grid.len) {
        return 0;
    }
    return grid[@intCast(point.y)][@intCast(point.x)];
}

fn isDigit(grid: [][]const u8, point: Point) bool {
    return std.ascii.isDigit(getChar(grid, point));
}
fn isGear(grid: [][]const u8, point: Point) bool {
    return getChar(grid, point) == '*';
}
...
pub fn main() !void {
    ...
    const result = run(input, allocator) catch unreachable;
    std.debug.print("Result: {any}\n", .{result});
}

test "example" {
    const x = run(example, std.testing.allocator) catch unreachable;
    try std.testing.expectEqual(@as(u32, 4361), x.total);
    try std.testing.expectEqual(@as(u32, 467835), x.gearRatio);
}

test "check" {
    const x = run(check, std.testing.allocator) catch unreachable;
    std.debug.print("Result: {any}\n", .{x});
    try std.testing.expectEqual(@as(u32, 13), x.total);
}
