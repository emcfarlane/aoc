...
fn run(src: []const u8, comptime allowed: anytype, allocator: std.mem.Allocator) !Result {
    var lines = std.mem.splitSequence(u8, src, "\n");
    var result: Result = std.mem.zeroInit(Result, .{});
    var gameID: u32 = 0;
    while (lines.next()) |line| {
        ...
        const game = line[idx + 1 ..]; // Game x: ...
        std.debug.print("{s}\n", .{game});
        var isOkay = true;
        var gameLines = std.mem.splitSequence(u8, game, ";");
        while (gameLines.next()) |gameLine| {
            var draw = std.mem.tokenizeAny(u8, gameLine, " ,");
            while (draw.peek() != null) {
                const countStr = draw.next().?;
                const color = draw.next().?;
                std.debug.print("{s} {s}\n", .{ countStr, color });
                const count = try std.fmt.parseInt(u32, countStr, 10);
                std.debug.print("{s} {d}\n", .{ color, count });
                const maxCount = allowed.get(color) orelse 0;
                if (count > maxCount) {
                    isOkay = false;
                }
                // Part 2
                const currentCount = map.get(color) orelse 0;
                if (currentCount < count) {
                    try map.put(color, count);
                }
            }
        }
        if (isOkay) {
            std.debug.print("Game {d} is okay\n", .{gameID});
            result.Total += gameID;
        } else {
            std.debug.print("Game {d} is not okay\n", .{gameID});
        }
        var power: u32 = 1;
        var it = map.iterator();
        while (it.next()) |kv| {
        ...
        std.debug.print("Game {d} power: {d}\n", .{ gameID, power });
        result.Power += power;
    }
    return result;
}
...
pub fn main() !void {
    ...
    const game = std.ComptimeStringMap(u32, .{
        .{ "red", 12 },
        .{ "green", 13 },
        .{ "blue", 14 },
    });
    const result = run(input, game, allocator) catch unreachable;
...
test "example" {
    ...
    const x = run(example, game, std.testing.allocator) catch unreachable;
    try std.testing.expectEqual(@as(u32, 8), x.Total);
    try std.testing.expectEqual(@as(u32, 2286), x.Power);
}
